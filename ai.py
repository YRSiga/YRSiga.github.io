...
def generate_recommendations(df, nutrient_predictions):
    ...
    for idx, row in enumerate(processed_data.itertuples(), start=1):
        recommendation = f"<br>Recommendation {idx}:<br>"
        recommendation += f"<br>Timestamp: {row.timestamp}<br>"
        recommendation += f"Potassium (K): {row.K}<br>"
        recommendation += f"Nitrogen (N): {row.N}<br>"
        recommendation += f"Phosphorus (P): {row.P}<br>"
        recommendation += f"Soil Moisture: {row[5]}<br>"
        ...
        if nutrient_predictions[idx - 1] == 1:
